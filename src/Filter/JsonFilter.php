...
final class JsonFilter extends AbstractContextAwareFilter implements JsonFilterInterface
{
    ...
    private function addNumericFilter(
        ...
        if (1 === count($values)) {
            ...
            $queryBuilder
                ->andWhere("JSON_EXTRACT({$alias}.{$jsonColumn}, '$.{$jsonKey}') = :{$valueParameter}")
                ->setParameter(
                    $valueParameter,
                    $values[0],
    ...
    private function addStringFilterWhereByStrategy(
        ...
        switch ($strategy) {
            ...
            case self::STRATEGY_EXACT:
                $queryBuilder
                    ->andWhere(
                        "JSON_EXTRACT({$jsonColumnWithAlias}, '$.{$jsonKey}') = :{$valueParameter}"
                    )
            ...
            case self::STRATEGY_PARTIAL:
                $queryBuilder
                    ->andWhere(
                        "JSON_EXTRACT({$jsonColumnWithAlias}, '$.{$jsonKey}') LIKE "
